...
\usepackage{multirow}
% to insert .bib file directly in the current file if needed
...
% 5.14 inches = \textwidth

\begin{document}
	
	\title{Tensor decomposition and forecast for multidimensional time series}
	
	\author*[1]{\fnm{Kirill} \sur{Semkin}}\email{semkin.ki@phystech.edu}
	\author*[2]{\fnm{Vadim} \sur{Strijov}}\email{strijov@ccas.ru}
	
	\affil*[1]{\orgname{Forecsys}}
	\affil*[2]{\orgname{Forecsys}}
	...
	The main object of the paper is multidimensional time series --- a set of two or more of them $ \{x_i(t)\}_{i=1}^m $ where $ t \in 1 \ldots N $. %By series decomposition we mean additive expansion of each signals into several components.
	...
	Two sets of data will be involved in experiment. First, electricity consumption paired with its generation price (fig. \ref{fig:electr_data}). Second, six signals from the motion sensors --- accelerometer and gyroscope measurements for walking movements (fig. \ref{fig:motion_data}). We assume signals' inter-dependency based on economical/physical reasons as it is important premise for our approach (see section \ref{sec:tssa_method}). Individual series has a length $ \approx 3 \cdot 10^3 $  and $ \approx 1.5 \cdot 10^3 $ where last $ 20\% $ are deferred test samples for prediction evaluation.
